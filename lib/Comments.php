 ...
 * @license GNU General Public License 2.0
 */
class sapphire_Widget_Comments_Archive extends Widget_Abstract_Comments
{
     /**
     * 当前页
     *
     ...
     * @return void
     */
private function get_Comment_MetaInfo($agent){
      $info_arr=[];
       if (preg_match('/MSIE\s([^\s|;]+)/i', $agent, $regs)) {
            $info_arr['Browser'] = '<i class="fa fa-internet-explorer"></i>';
        } else if (preg_match('/FireFox\/([^\s]+)/i', $agent, $regs)) {
           $info_arr['Browser'] = '<i class="fa fa-firefox"></i>';
        } else if (preg_match('/Chrome([\d]*)\/([^\s]+)/i', $agent, $regs)) {
            $info_arr['Browser'] = '<i class="fa fa-chrome"></i>';
        } else if (preg_match('/safari\/([^\s]+)/i', $agent, $regs)) {
            $info_arr['Browser'] = '<i class="fa fa-safari"></i>';
        } else if (preg_match('/Opera[\s|\/]([^\s]+)/i', $agent, $regs)) {
            $info_arr['Browser'] = '<i class="fa fa-opera"></i>';
        } else {
            $info_arr['Browser'] = '<i class="fa fa-question"></i>';
        }
      
       if (preg_match('/win/i', $agent)) {
           $info_arr['OS']= '<i class="fa fa-windows"></i>';
        } else if (preg_match('/android/i', $agent)) {
            $info_arr['OS'] = '<i class="fa fa-android"></i>';
        } else if (preg_match('/linux/i', $agent)) {
             $info_arr['OS']= '<i class="fa fa-linux"></i>';
        } else if (preg_match('/mac/i', $agent)) {
            $info_arr['OS'] = '<i class="fa fa-apple"></i>';
        } else if (preg_match('/iphone/i', $agent)) {
           $info_arr['OS']= '<i class="fa fa-apple"></i>';
        } else if (preg_match('/ipad/i', $agent)) {
            $info_arr['OS'] = '<i class="fa fa-apple"></i>';
        } else {
            $info_arr['OS'] = '<i class="fa fa-laptop"></i>';
        }
    ...
    private function threadedCommentsCallback()
	{
        ...
        ?>
        <div class="comment-wrap" style="border: 1px solid #4d72a9;padding: 22px;margin: 10px;">
            <div id="<?php $this->theId(); ?>">

         <?php
         /* 
         <div id="<?php $this->theId(); ?>" class="
         //这一段是登陆后的样式表，这里暂时不用，后续版本再看添加
         $commentClass = '';
            if ($this->authorId) {
     ...
     "></i>&nbsp<b><?php $this->author(); ?></b> • <i class="fa fa-clock-o"></i>&nbsp<?php echo Utils::formatDate($this->created,'NATURAL');$meta_info=self::get_Comment_MetaInfo($this->agent); ?> • <?php echo $meta_info['Browser'];  ?> | <?php echo $meta_info['OS'];  ?> 
                      
                      <?php //Utils::exportCommentMeta($this);?>
                  
                  </span>
                    <div class="comment-content"><?php echo $this->getParent(); 
      echo Utils::parseBiaoQing($this->content); ?></div>
                </div>
            </div>
